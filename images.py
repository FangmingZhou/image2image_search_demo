...
def im2path(collection, name, big=True):
    img_dir = os.path.join(rootpath, collection, 'ImageData128x128')
    img_dir = img_dir.replace('128x128','') if big else img_dir
    ext = '.JPEG' if collection.startswith('imagenet') else '.jpg'

    subfolder = '_'.join(name.split('_')[:-1])
    return os.path.join(img_dir, subfolder, os.path.splitext(name)[0] + ext)
